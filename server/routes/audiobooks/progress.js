...
function register(router, { db, authenticateToken }) {
   ...
   */
  async function queueNextInSeries(userId, finishedAudiobookId) {
    try {
      // Get the finished audiobook's series info
      const finishedBook = await dbGet(
        'SELECT series, series_position, series_index FROM audiobooks WHERE id = ?',
        [finishedAudiobookId]
      );
      if (!finishedBook || !finishedBook.series) {
  ...
  router.delete('/:id/progress', authenticateToken, async (req, res) => {
    const audiobookId = req.params.id;
    const userId = req.user.id;
  ...
  router.get('/:id/previous-book-status', authenticateToken, async (req, res) => {
    ...
    try {
      ...
      const previousBook = await dbGet(
