...
function register(router, { db, authenticateToken, requireAdmin, normalizeGenres, GENRE_MAPPINGS, DEFAULT_GENRE_METADATA }) {
  ...
  const cleanProgressFields = (book) => {
    delete book.progress_position;
    delete book.progress_completed;
  };

  // Get all series with cover IDs
  router.get('/meta/series', authenticateToken, async (req, res) => {
    try {
  ...
  router.get('/meta/authors', authenticateToken, async (req, res) => {
    try {
  ...
  router.get('/meta/genres', authenticateToken, async (req, res) => {
    try {
  ...
  router.get('/meta/recent', authenticateToken, async (req, res) => {
    ...
    try {
      ...
      const result = audiobooks.map(transformProgress);
      result.forEach(cleanProgressFields);
      res.json(result);
    } catch (_err) {
      res.status(500).json({ error: 'Internal server error' });
    }
  ...
  router.get('/meta/in-progress', authenticateToken, async (req, res) => {
    const limit = parseInt(req.query.limit) || 10;
    const userId = req.user.id;
    ...
    try {
      ...
      const result = audiobooks.map(book => ({
        ...book,
        normalized_genre: normalizeGenres(book.genre),
        is_favorite: !!book.is_favorite,
  ...
  router.get('/meta/up-next', authenticateToken, async (req, res) => {
    const limit = parseInt(req.query.limit) || 10;
    const userId = req.user.id;
  ...
  router.get('/meta/finished', authenticateToken, async (req, res) => {
    const limit = parseInt(req.query.limit) || 10;
    const userId = req.user.id;
