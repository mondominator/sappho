...
/**
 * Extract narrator from composer, MP4/iTunes, ID3v2, and Vorbis tags.
 */
function extractNarrator(nativeTags, common) {
...
/**
 * Extract title from common tags, native tags, or filename/directory fallback.
 * Also extracts series info from title if not already found in tags.
 * Returns { title, series, seriesPosition } where series/seriesPosition are only
 * set if extracted from the title pattern.
 */
function extractTitle(common, nativeTags, iTunesTags, filePath) {
  ...
  // Try to extract series from title pattern
  let titleSeries = null;
  let titleSeriesPosition = null;
  if (title) {
    ...
    if (seriesMatch) {
      titleSeries = seriesMatch[1].trim();
      const position = parseFloat(seriesMatch[2]);
      if (!isNaN(position)) {
        titleSeriesPosition = position;
      }
    }
  }

  return { title, titleSeries, titleSeriesPosition };
}

/**
 * Extract description from iTunes long description, common.description,
 * or comment tags (only if they look like prose, not chapter listings).
 */
function extractDescription(common, nativeTags, iTunesTags) {
...
/**
 * Extract extended metadata fields (tags, publisher, copyright, ISBN, ASIN,
 * language, rating, abridged, subtitle, published_year) from iTunes/MP4 tags
 * with common tag fallbacks.
 */
function extractExtendedMetadata(iTunesTags, common) {
  let tags = null;
  let publisher = null;
  ...
  return { tags, publisher, copyright_year, isbn, asin, language, rating, abridged, subtitle, published_year };
}

/**
 * Extract author from common tags with native tag fallbacks.
 */
function extractAuthor(common, nativeTags, iTunesTags) {
...
async function extractFileMetadata(filePath) {
  try {
    ...
    // Delegate to focused extraction functions
    let { series, seriesPosition } = extractSeriesInfo(nativeTags, common, iTunesTags);
    const narrator = extractNarrator(nativeTags, common);
    const { title, titleSeries, titleSeriesPosition } = extractTitle(common, nativeTags, iTunesTags, filePath);

    // Use series from title only if tags didn't provide one
    ...
    return {
      ...
      copyright_year: extended.copyright_year,
      asin: extended.asin,
